...
class EstacionamientoSupabaseUser extends BaseAuthUser {
  EstacionamientoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> estacionamientoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EstacionamientoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
